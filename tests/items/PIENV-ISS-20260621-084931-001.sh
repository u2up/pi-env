...
test_dir_exists coordination/todos
test_grep '`TODO`: `todo`' coordination/docs/ITEM_FORMAT.md
test_grep '`TODO` for `todo`' coordination/AGENTS.md
test_grep '`TODO` for todo' coordination/.pi/skills/agent-coordination/SKILL.md
...
test_grep '^status: active$' "coordination/$todo_path"
test_grep '^body: |-$' "coordination/$todo_path"
assert_no_top_level_history "coordination/$todo_path"

todo_id="$(grep '^id: ' "coordination/$todo_path" | sed 's/^id: //')"
todo_list="$(agent-coord-list --coord-dir coordination todos active)"
printf '%s\n' "$todo_list" \
  | grep -Eq "^$todo_id[[:space:]]+active[[:space:]]+Example TODO$" \
  || test_fail 'TODO item was not listed by todos active'

if agent-coord-new --coord-dir coordination --type tdo "Bad alias" \
...
if agent-coord-list --coord-dir coordination tdo \
