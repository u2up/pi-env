...
help_output="$(agent-coord-new --help)"
...
if printf '%s\n' "$help_output" | grep -q 'project-root layout only'; then
  printf 'agent-coord-new help still says project-root layout only\n' >&2
  exit 1
fi
if printf '%s\n' "$help_output" | grep -q 'root item folders such as issues/'; then
  printf 'agent-coord-new help still lists root issues folders as primary layout\n' >&2
  exit 1
fi
