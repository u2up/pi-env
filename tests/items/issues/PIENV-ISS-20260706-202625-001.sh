...
bash scripts/pi-env-bwrap --help | grep -q 'pi-env-bwrap --shell'
...
bash scripts/pi-env-bwrap --shell >/dev/null
if grep -qx -- '--tools' "$fake_root/trace" || grep -qx -- '--continue' "$fake_root/trace"; then
  echo "pi-env-bwrap --shell without bash args must not use default Pi args" >&2
  exit 1
fi
...
assert.match(readme, /`pi-env-bwrap --shell \[--\] \[bash args\.\.\.\]`/);
...
assert.match(requirements, /#### CMD-021 `pi-env-bwrap` shell mode/);
