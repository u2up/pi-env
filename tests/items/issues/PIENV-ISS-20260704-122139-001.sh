...
(
  ...
  . "$repo_root/scripts/agent-coord-lib.sh"
...
agent-coord-cat --coord-dir "$coord_dir" ALPHA-ISS-1 | grep -q '^id: ALPHA-ISS-1$'

# New issues in a repo-scoped coordination domain require a repo namespace.
if agent-coord-new --coord-dir "$coord_dir" "Root write" >"$tmp/new.out" 2>"$tmp/new.err"; then
  printf 'agent-coord-new unexpectedly wrote a root issue\n' >&2
  exit 1
fi
grep -q -- '--repo-id' "$tmp/new.err"
new_path="$(agent-coord-new --coord-dir "$coord_dir" --repo-id alpha "Repo write" | tail -n 1)"
case "$new_path" in
...
agent-coord-done --coord-dir "$coord_dir" --agent-id agent-a --role developer \
...
if agent-coord-lint --coord-dir "$coord_dir" --project-root "$repo_root" >"$tmp/lint.out" 2>"$tmp/lint.err"; then
  printf 'agent-coord-lint unexpectedly accepted duplicate issue IDs\n' >&2
  exit 1
fi
